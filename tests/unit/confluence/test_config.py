...
def test_from_env_missing_credentials():
    """Test that from_env raises ValueError when credentials are missing."""
    with patch.dict(
        os.environ,
        {
        ...
        with pytest.raises(
            ValueError,
            match="需要CONFLUENCE_USERNAME和CONFLUENCE_API_TOKEN",
        ):
            ConfluenceConfig.from_env()
